...
class Holistic(AbstDetector):
    ...
    def show(self, image) -> np.array:
        ...
        if self.results.pose_landmarks:
            self.__draw_keypoints_connections(image, self.results.pose_landmarks.landmark, circle_radius=5, connections=mp.solutions.pose.POSE_CONNECTIONS)

        # left hand pose
        if self.results.left_hand_landmarks:
            self.__draw_keypoints_connections(image, self.results.left_hand_landmarks.landmark, circle_radius=5, connections=mp.solutions.holistic.HAND_CONNECTIONS)
        
        # right hand pose
        if self.results.right_hand_landmarks:
            self.__draw_keypoints_connections(image, self.results.right_hand_landmarks.landmark, circle_radius=5, connections=mp.solutions.holistic.HAND_CONNECTIONS)
